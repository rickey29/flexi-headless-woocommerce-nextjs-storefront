...
describe('setErrorReporter', () => {
  ...
  it('should accept a reporter function', () => {
  ...
  it('should not call reporter in non-production environment', () => {
    const reporter = vi.fn();
    setErrorReporter(reporter);

    logError('Test error', new Error('test'));

    // In test environment (not production), reporter should not be called
    expect(reporter).not.toHaveBeenCalled();
  });
});

describe('setErrorReporter in production', () => {
  ...
  it('should call reporter in production environment', async () => {
    (process.env as Record<string, string | undefined>).NODE_ENV = 'production';

    const { logError: logErrorProd, setErrorReporter: setReporterProd } = await import('../logger');

    const reporter = vi.fn();
    setReporterProd(reporter);

    const testError = new Error('test error');
    logErrorProd('Production error', testError, { request_id: 'req_123' });

    expect(reporter).toHaveBeenCalledWith('Production error', testError, {
      request_id: 'req_123',
    });

    // Cleanup
    setReporterProd(null);
  });
  ...
  it('should catch and log reporter errors', async () => {
    ...
    setReporterProd(failingReporter);

    // Should not throw even when reporter fails
    expect(() => {
      logErrorProd('Error with failing reporter', new Error('original'));
    }).not.toThrow();

    // Should have logged the reporter failure
    expect(consoleErrorSpy).toHaveBeenCalledWith(
      '[logger] Error reporter failed:',
      expect.any(Error),
    );
...
describe('logInfo when shouldLog returns true', () => {
  ...
  it('should call console.log when LOG_LEVEL allows info', async () => {
    ...
    const { logInfo: logInfoDev } = await import('../logger');
...
describe('logDebug when shouldLog returns true', () => {
  ...
  it('should call console.log when LOG_LEVEL is debug', async () => {
    ...
    const { logDebug: logDebugDev } = await import('../logger');
    const consoleLogSpy = vi.spyOn(console, 'log').mockImplementation(() => {});
...
describe('JSON Logging Mode', () => {
  ...
  it('should output JSON when LOG_JSON is true', async () => {
    ...
    const { logWarn: logWarnJson } = await import('../logger');
  ...
  it('should include request ID in JSON output when set', async () => {
    ...
    const { logWarn: logWarnJson, setRequestId: setReqId } = await import('../logger');
    const consoleWarnSpy = vi.spyOn(console, 'warn').mockImplementation(() => {});
  ...
  it('should output JSON for error logs in JSON mode', async () => {
    ...
    const { logError: logErrorJson } = await import('../logger');
    const consoleErrorSpy = vi.spyOn(console, 'error').mockImplementation(() => {});
