 ...
 * - Backward-compatible error reporter hook
 *
 * @since 1.0.0
 * @updated 1.1.0 - Added JSON logging, request tracing, Sentry integration
 */

import { config, shouldLog, type LogLevel } from '../config/env';
...
/**
 * Error reporter function type for external error tracking services
 * @deprecated Use Sentry integration via SENTRY_DSN instead
 */
export type ErrorReporter = (message: string, error: Error | unknown, context?: LogContext) => void;

/**
 * Registered error reporter for external error tracking
 * @deprecated Use Sentry integration via SENTRY_DSN instead
 */
let errorReporter: ErrorReporter | null = null;

/**
 * Register an error reporter for external error tracking
 * @deprecated Use Sentry integration via SENTRY_DSN instead
 *
 * Example usage:
 * ```typescript
 ...
 *
 * setErrorReporter((message, error, context) => {
 *   // Send to your error tracking service
 *   myErrorService.capture(error, { message, ...context });
 * });
 * ```
 */
...
export function logError(message: string, error?: Error | unknown, context?: LogContext): void {
  ...
  // Also send to registered error reporter if available (backward compatibility)
  if (errorReporter && config.IS_PRODUCTION) {
    try {
      errorReporter(message, error, context);
    } catch (reporterError) {
      console.error('[logger] Error reporter failed:', reporterError);
    }
  }
}
